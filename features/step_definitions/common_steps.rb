...
Then(/^I land on Search screen$/) do
  ...
  # expect(result).to be == 1
  expect(find_element(:class_name, "UIASearchBar").text).to be == "Search Wikipedia"
end
...
When(/^I tap on (\d+)(?:st|nd|rd|th)? result$/) do |index|
  find_element(:xpath, "//UIAApplication[1]/UIAWindow[1]/UIACollectionView[1]/UIACollectionCell[#{index}]/UIAStaticText[1]").click
end
...
And(/^I verify that article title include "([^"]*)"$/) do |expected_result|
  ...
  expect(element.text.downcase).to be == expected_result
end
